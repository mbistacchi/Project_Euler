# -*- coding: utf-8 -*-
"""
Created on Sun Jan 20 12:55:30 2019
...
for i in range(20):
    for j in range(20):
        ...
        if i < 17:
            temp = m[i][j] * m[i+1][j] * m[i+2][j] * m[i+3][j] #vertical mult
            temp5.append(temp)
            
        if j < 17:
            temp2 = m[i][j] * m[i][j+1] * m[i][j+2] * m[i][j+3] # hortizontal mult
            temp5.append(temp2)
            
        if i < 17 and j < 17:
            temp3 = m[i][j] * m[i+1][j+1] * m[i+2][j+2] * m[i+3][j+3] # down-right diag mult
            temp5.append(temp3)
            
        if j > 2 and i < 17:
            temp4 = m[i][j] * m[i+1][j-1] * m[i+2][j-2] * m[i+3][j-3] # down-left diag mult
            temp5.append(temp4)
